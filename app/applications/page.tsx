...
function AppCard({ app, index }: { app: typeof myApps[0]; index: number }) {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-2xl p-8 shadow-sm border border-gray-100 transition-all duration-500 cursor-pointer group relative overflow-hidden ${
        isVisible 
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full group-hover:bg-gray-200 transition-colors duration-300">
          {app.tag}
        </span>
      </div>
      
      <h2 className="text-xl font-bold mb-3 text-gray-900 group-hover:text-teal-600 transition-colors duration-300">
        {app.name}
      </h2>
      ...
      {/* Arrow indicator */}
      <div className="absolute bottom-6 right-6 text-gray-400 group-hover:text-teal-500 transition-all duration-300 transform group-hover:translate-x-2">
...
function BackButton() {
  ...
  return (
    <button
      ...
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      className="inline-flex items-center gap-2 bg-gray-900 text-white px-8 py-3 rounded-full hover:bg-gray-800 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
    >
      <ArrowLeft 
        size={18} 
